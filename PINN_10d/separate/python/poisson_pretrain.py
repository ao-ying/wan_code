...
from poisson_2d_sep import PINN, Residual # GPU会受poisson_separate_nx1.py控制。
...
class PINN_Main(nn.Module):
    def __init__(self, Layers, num_interior, num_boundary, boundary_weight, lb_X, ub_X, num_res, in_dim, dim_u, pre_train_path):
    ...
    def initianl_NN(self):
        ...
        for i in range(self.num_channel):
            ...
            self.nns.append(temp_model)
        
        # 参数设置
        for i in range(self.num_channel):
            self.params = self.params + self.nns[i].params
    ...
    def neural_net(self, X):
        ...
        out_list = []
        for i in range(self.num_channel): 
            H = self.nns[i].neural_net(In[:, self.in_list[i] : self.in_list[i+1]])
            out_list.append(H)
    ...
    def he_net(self, X):
        ...
        u_b = self.neural_net(X_b) # self.num_boundary * self.dim_x
        boundary = u_b - 0
...
if __name__ == "__main__":
    ...
    if train:
        ...
        # 声明神经网络实例
        model = PINN_Main(Layers, num_interior, num_boundary, boundary_weight, lb_X, ub_X, num_res, in_dim, dim_u, pre_train_path)
