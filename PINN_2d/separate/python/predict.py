...
# points = np.loadtxt(path + "../data/points.txt", dtype = float, delimiter=" ") # 也可以给定测试集，测试集需要是(N,2)维数据
X_pred = points
X_pred = torch.from_numpy(X_pred).float().to(device_cpu)

# 得到预测值并画图
model2 = torch.load(path + '../output/5*3*128_boundary_100000/network.pkl', map_location=device_cpu)
