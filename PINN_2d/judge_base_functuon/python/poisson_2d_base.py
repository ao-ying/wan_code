...
def get_rhs(X_train, num_nn, dim_x, num_interior, num_boundary, output_path):
    ...
    if num_nn == 0:
        ...
        np.savetxt(output_path + "rhs_%d.txt" % num_nn, rhs, fmt="%s",delimiter=' ')
    else:
        rhs = np.loadtxt(output_path + "rhs_%d.txt" % num_nn, dtype = float, delimiter=' ')
        rhs = np.reshape(rhs, (rhs.shape[0],1)) # 注意这里维度要增加
        
    return rhs

# 计算第(num_nn+1)个神经网络对应的右端项
def Cal_rhs(num_nn, output_path):
    # 载入上一个模型的rhs
    rhs = np.loadtxt(output_path + "rhs_%d.txt" % num_nn, dtype = float, delimiter=' ')
    rhs = np.reshape(rhs, (rhs.shape[0],1))
    # 计算当前模型的rhs
    model2 = torch.load(output_path + 'network_%d.pkl' % num_nn, map_location=device) 
    temp = model2.cal_rhs()
    # 得到最终的rhs并保存
    rhs = rhs + temp
    np.savetxt(output_path + "rhs_%d.txt" % (num_nn + 1), rhs, fmt="%s",delimiter=' ') 
...
if __name__ == "__main__":
    ...
    if train:
        ...
        # 依次训练多个神经网络
        for num_nn in range(max_nn):
            print("***** Neural network %d *****" % num_nn)
            
            # 右端项
            rhs = get_rhs(X_train_np, num_nn, dim_x, num_interior, num_boundary, output_path) # 这里暂时设为0
            rhs = torch.from_numpy(rhs).float().to(device)
            if num_nn != 0: # 递减Layers
                Layers = [int(Layers[i] / 2) for i in range(len(Layers))]
            ...
            for it in range(epochs):
                ...
                if loss_val < min_loss:
                    torch.save(model, output_path + 'network_%d.pkl' % num_nn) 
                    min_loss = loss_val
            ...
            np.savetxt(output_path + "loss_%d.txt" % num_nn, loss_list, fmt="%s",delimiter=' ')
            ...
            # 保存loss曲线
            loss_list = np.loadtxt(output_path + "loss_%d.txt" % num_nn, dtype = float, delimiter=' ')
            plt.semilogy(loss_list)
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            if align:
                plt.ylim(lb_loss, ub_loss) # 与BiPINN统一量纲
                plt.savefig(output_path + 'loss_%d_aligned.pdf' % num_nn, format="pdf", dpi=300, bbox_inches="tight")
            else:
                plt.savefig(output_path + 'loss_%d.pdf' % num_nn, format="pdf", dpi=300, bbox_inches="tight")
            ...
            model2 = torch.load(output_path + 'network_%d.pkl' % num_nn, map_location=device) 
            u_pred = model2.predict(X_pred).flatten()
            # 如果不是第一个神经网络，则需要把前面的预测值加进来。
            if num_nn != 0:
                u_previous = np.loadtxt(output_path + "u_pred_%d.txt" % (num_nn - 1), dtype = float, delimiter=' ')
                u_pred = u_pred + u_previous
            np.savetxt(output_path + "u_pred_%d.txt" % num_nn, u_pred, fmt="%s",delimiter=' ')
            ...
            plt.title('$u$(pred)')
            if align:
                plt.savefig(output_path + "u_pred_%d_aligned.png" % num_nn, format="png", dpi=300, bbox_inches="tight")
            else:
                plt.savefig(output_path + "u_pred_%d.png" % num_nn, format="png", dpi=300, bbox_inches="tight")
            
            # 计算相对误差并判断终止条件
            u_error = np.linalg.norm((u_truth - u_pred),ord=2) / (np.linalg.norm(u_truth, ord=2))
            print("Neural network %d: Relative error of u is %.8f" % (num_nn, u_error))
            if u_error < error or num_nn == max_nn - 1: 
                break
            
            # 计算右端项
            Cal_rhs(num_nn, output_path)
